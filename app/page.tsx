...
export default function Home() {
  const features = [
    { icon: <BookOpen size={24} />, title: 'Comprehensive Courses', desc: 'Tajweed, Hifz, Tafsir, Fiqh, and Islamic Studies — structured from beginner to advanced.', color: '#22c55e' },
    { icon: <Video size={24} />, title: 'Live Online Classes', desc: 'Interactive face-to-face sessions with qualified teachers via our integrated video platform.', color: '#4f8ef7' },
    { icon: <Star size={24} />, title: 'Skilled Instructors', desc: 'Certified Quran teachers with years of experience and deep scholarly knowledge.', color: '#f59e0b' },
    { icon: <Globe size={24} />, title: 'Worldwide Access', desc: 'Learn from the comfort of your home — wherever you are in the world.', color: '#8b5cf6' },
    { icon: <CheckCircle size={24} />, title: 'Progress Tracking', desc: 'Assignments, grades, and attendance tracked automatically — full transparency.', color: '#22c55e' },
    { icon: <Award size={24} />, title: 'Authentic Teaching', desc: 'Accurate, meaningful Quranic education built on traditional methods and modern tools.', color: '#ef4444' },
  ]
    ...
    <div style={{ background: 'var(--bg-primary)', color: 'var(--text-primary)', fontFamily: "'Inter', sans-serif", minHeight: '100vh' }}>
      ...
      <section style={{ position: 'relative', overflow: 'hidden', padding: '100px 24px 80px' }}>
        ...
        <div style={{ maxWidth: 900, margin: '0 auto', textAlign: 'center', position: 'relative', zIndex: 1 }}>
          ...
          <p style={{ fontSize: 'clamp(1rem, 2vw, 1.2rem)', color: 'var(--text-secondary)', maxWidth: 620, margin: '0 auto 40px', lineHeight: 1.7 }}>
          ...
          <div style={{ display: 'flex', gap: 14, justifyContent: 'center', flexWrap: 'wrap' }}>
            <Link href="/login" style={{ padding: '14px 32px', background: 'linear-gradient(135deg, #22c55e 0%, #16a34a 100%)', color: '#fff', borderRadius: 10, fontWeight: 700, fontSize: '1rem', textDecoration: 'none', display: 'inline-flex', alignItems: 'center', gap: 8, boxShadow: '0 4px 24px rgba(34,197,94,0.3)' }}>
      ...
      <section style={{ padding: '20px 24px 80px' }}>
        <div style={{ maxWidth: 1100, margin: '0 auto' }}>
          <div style={{ textAlign: 'center', marginBottom: 52 }}>
            <h2 style={{ fontSize: 'clamp(1.75rem, 4vw, 2.5rem)', fontWeight: 800, letterSpacing: '-0.02em', marginBottom: 12 }}>Why Choose Us?</h2>
            <p style={{ color: 'var(--text-muted)', fontSize: '1.0625rem' }}>A modern platform built around authentic Quranic education</p>
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: 20 }}>
            {features.map(f => (
              <div key={f.title} style={{ padding: 28, background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 16 }}>
                <div style={{ width: 52, height: 52, borderRadius: 14, background: `${f.color}18`, color: f.color, display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: 18 }}>{f.icon}</div>
      ...
      <section style={{ padding: '20px 24px 80px' }}>
        <div style={{ maxWidth: 1100, margin: '0 auto' }}>
          <div style={{ textAlign: 'center', marginBottom: 48 }}>
            <h2 style={{ fontSize: 'clamp(1.75rem, 4vw, 2.5rem)', fontWeight: 800, letterSpacing: '-0.02em', marginBottom: 12 }}>Our Courses</h2>
            <p style={{ color: 'var(--text-muted)', fontSize: '1rem' }}>Structured programs for every level</p>
          </div>
          ...
          <div style={{ textAlign: 'center', marginTop: 36 }}>
            <Link href="/login" style={{ padding: '13px 30px', background: 'rgba(34,197,94,0.12)', color: '#22c55e', border: '1px solid rgba(34,197,94,0.3)', borderRadius: 10, fontWeight: 600, textDecoration: 'none', display: 'inline-flex', alignItems: 'center', gap: 8 }}>
      ...
      {/* CTA */}
      <section style={{ padding: '20px 24px 80px' }}>
        <div style={{ maxWidth: 1100, margin: '0 auto', padding: '60px 48px', background: 'linear-gradient(135deg, rgba(34,197,94,0.1) 0%, rgba(245,158,11,0.08) 100%)', border: '1px solid rgba(34,197,94,0.2)', borderRadius: 24, textAlign: 'center' }}>
          <div style={{ fontSize: '2.5rem', marginBottom: 16 }}>📖</div>
          <h2 style={{ fontSize: 'clamp(1.5rem, 3.5vw, 2.25rem)', fontWeight: 800, marginBottom: 14, letterSpacing: '-0.02em' }}>Ready to Begin Your Quran Journey?</h2>
          ...
          <div style={{ display: 'flex', gap: 14, justifyContent: 'center', flexWrap: 'wrap' }}>
            ...
            <a href="https://latifiaquranonline.com/" target="_blank" rel="noopener noreferrer"
              style={{ padding: '14px 28px', background: 'rgba(34,197,94,0.1)', color: '#22c55e', border: '1px solid rgba(34,197,94,0.3)', borderRadius: 10, fontWeight: 600, fontSize: '1rem', textDecoration: 'none', display: 'inline-flex', alignItems: 'center', gap: 8 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: 40, marginBottom: 48 }}>
            <div>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 14 }}>
                ...
                <span style={{ fontWeight: 700 }}>Latifia Quraner Alo</span>
              </div>
            ...
            <div>
              <p style={{ fontWeight: 600, marginBottom: 16, fontSize: '0.875rem' }}>Platform</p>
              {[{ label: 'Student Login', href: '/login' }, { label: 'Teacher Login', href: '/login' }, { label: 'Create Account', href: '/signup' }].map(l => (
            ...
            <div>
              <p style={{ fontWeight: 600, marginBottom: 16, fontSize: '0.875rem' }}>Courses</p>
              {['Quran Recitation', 'Tajweed Rules', 'Hifzul Quran', 'Tafsir & Meaning', 'Islamic Studies'].map(c => (
            ...
            <div>
              <p style={{ fontWeight: 600, marginBottom: 16, fontSize: '0.875rem' }}>Contact</p>
              <a href="https://latifiaquranonline.com/" target="_blank" rel="noopener noreferrer"
