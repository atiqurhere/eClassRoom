...
import {
  ...
  GraduationCap, ClipboardList, Eye, BookMarked, Monitor
} from 'lucide-react'
    ...
    <aside className="flex flex-col h-full flex-shrink-0" style={{ width: 240, background: 'var(--bg-secondary)', borderRight: '1px solid var(--border)' }}>
      ...
      <div className="px-5 py-5">
        <div className="flex items-center gap-3">
          <div className="w-9 h-9 rounded-xl flex items-center justify-center flex-shrink-0" style={{ background: 'linear-gradient(135deg, #4f8ef7 0%, #8b5cf6 100%)' }}>
            <GraduationCap size={20} color="white" />
          </div>
          <div>
            <p className="text-sm font-bold" style={{ color: 'var(--text-primary)' }}>E-Classroom</p>
            <p className="text-xs font-medium" style={{ color: roleColors[role] || 'var(--accent-blue)' }}>{roleLabelMap[role]}</p>
          </div>
        </div>
      </div>
